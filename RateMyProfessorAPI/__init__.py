...
sql_cmd = """UPDATE instructors SET avg_rating = ?, avg_helpful = ?, num_ratings = ?, avg_easy_score = ?, avg_clarity_score = ? WHERE person_id = ?;"""
...
def get_rating(person_id: str, first_name: str, last_name: str, rest_of_name: str, surname: str, data_stream: List):
	...
	if got_data and response["docs"][0]["total_number_of_ratings_i"] > 0:
		values = (response["docs"][0]["averageratingscore_rf"],
		...
		values = (None, None, None, None, None, person_id)
	data_stream.append(values)


def append_rmp_info(profs: List, cursor: sqlite3.Cursor, append_function: Callable = print,
	...
	append_function("Committing Rate My Professor data...")
	for data_bit in data_stream:
		cursor.execute(sql_cmd, data_bit)
	replace_function("Committed Rate My Professor data")
